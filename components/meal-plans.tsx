...
const mealTypeColors: Record<MealType, string> = {
  breakfast: "bg-amber-100 text-amber-800 border-amber-200 dark:bg-amber-900 dark:text-amber-100 dark:border-amber-700",
  lunch: "bg-emerald-100 text-emerald-800 border-emerald-200 dark:bg-emerald-900 dark:text-emerald-100 dark:border-emerald-700",
  dinner: "bg-indigo-100 text-indigo-800 border-indigo-200 dark:bg-indigo-900 dark:text-indigo-100 dark:border-indigo-700",
  snack: "bg-sky-100 text-sky-800 border-sky-200 dark:bg-sky-900 dark:text-sky-100 dark:border-sky-700",
};
...
const MealPlans = ({ mealPlans }: MealPlansProps) => {
  ...
  const MealCard = ({ meal }: { meal: Meal }) => (
    <Card
      className="p-4 mb-4 cursor-pointer border rounded-xl group transition-all hover:shadow-lg dark:border-gray-700 dark:bg-gray-900"
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <h3 className="font-semibold text-lg mb-1 group-hover:text-indigo-600 transition-colors dark:text-white">
            {meal.name}
          </h3>
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-5xl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8">
        ...
        <Select value={view} onValueChange={setView}>
          <SelectTrigger className="w-[160px] sm:w-[180px] dark:bg-gray-800 dark:border-gray-700">
      ...
      <Card className="mb-6 overflow-hidden border rounded-xl shadow-sm dark:border-gray-700 dark:bg-gray-900">
        <div className="p-4 flex items-center justify-between border-b dark:border-gray-700 bg-white dark:bg-gray-800 sticky top-0 z-10">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => navigateDay("prev")}
            className="rounded-full hover:bg-indigo-50 hover:text-indigo-600 dark:hover:bg-indigo-700"
          >
        ...
        <ScrollArea className="h-[calc(100vh-280px)] sm:h-[calc(100vh-260px)] p-4 bg-gray-50 dark:bg-gray-900">
      ...
      <Dialog open={!!selectedMeal} onOpenChange={(open) => !open && setSelectedMeal(null)}>
        <DialogContent className="max-w-md sm:max-w-lg rounded-xl overflow-hidden">
          {selectedMeal && (
            <>
